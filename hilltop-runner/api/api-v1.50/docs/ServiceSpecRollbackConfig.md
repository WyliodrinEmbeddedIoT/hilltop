...
**failure_action** | Option<**FailureAction**> | Action to take if an rolled back task fails to run, or stops running during the rollback.  (enum: continue, pause) | [optional]
...
**order** | Option<**Order**> | The order of operations when rolling back a task. Either the old task is shut down before the new task is started, or the new task is started before the old task is shut down.  (enum: stop-first, start-first) | [optional]
