...
pub async fn secret_create(configuration: &configuration::Configuration, body: Option<models::SecretCreateRequest>) -> Result<models::IdResponse, Error<SecretCreateError>> {
...
pub async fn secret_delete(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<SecretDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;

    let uri_str = format!("{}/secrets/{id}", configuration.base_path, id=crate::apis::urlencode(p_path_id));
...
pub async fn secret_inspect(configuration: &configuration::Configuration, id: &str) -> Result<models::Secret, Error<SecretInspectError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
...
pub async fn secret_list(configuration: &configuration::Configuration, filters: Option<&str>) -> Result<Vec<models::Secret>, Error<SecretListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn secret_update(configuration: &configuration::Configuration, id: &str, version: i64, body: Option<models::SecretSpec>) -> Result<(), Error<SecretUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_query_version = version;
    let p_body_body = body;

    let uri_str = format!("{}/secrets/{id}/update", configuration.base_path, id=crate::apis::urlencode(p_path_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
