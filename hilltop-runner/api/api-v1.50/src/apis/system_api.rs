...
pub async fn system_auth(configuration: &configuration::Configuration, auth_config: Option<models::AuthConfig>) -> Result<models::SystemAuthResponse, Error<SystemAuthError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SystemAuthResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SystemAuthResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn system_data_usage(configuration: &configuration::Configuration, r#type: Option<Vec<String>>) -> Result<models::SystemDataUsageResponse, Error<SystemDataUsageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
