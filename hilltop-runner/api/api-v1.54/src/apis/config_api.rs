...
pub async fn config_create(configuration: &configuration::Configuration, body: Option<models::ConfigCreateRequest>) -> Result<models::IdResponse, Error<ConfigCreateError>> {
...
pub async fn config_delete(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<ConfigDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;

    let uri_str = format!("{}/configs/{id}", configuration.base_path, id=crate::apis::urlencode(p_path_id));
...
pub async fn config_inspect(configuration: &configuration::Configuration, id: &str) -> Result<models::Config, Error<ConfigInspectError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
...
pub async fn config_list(configuration: &configuration::Configuration, filters: Option<&str>) -> Result<Vec<models::Config>, Error<ConfigListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::Config&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn config_update(configuration: &configuration::Configuration, id: &str, version: i64, body: Option<models::ConfigSpec>) -> Result<(), Error<ConfigUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_query_version = version;
    let p_body_body = body;

    let uri_str = format!("{}/configs/{id}/update", configuration.base_path, id=crate::apis::urlencode(p_path_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
